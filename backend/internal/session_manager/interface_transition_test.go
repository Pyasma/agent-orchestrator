...
func TestInterfaceTransitionReportsNativeHistoryReplayFailure(t *testing.T) {
	tests := []struct {
		...
		{name: "load rejected", err: ports.ErrChatHistoryLoadRejected, code: "TARGET_HISTORY_LOAD_REJECTED", wantStarts: 1},
		...
		{name: "load rejected after unsettled wait", err: fmt.Errorf("wait for settled native conversation history: %w: %w",
			ports.ErrChatHistoryUnsettled, fmt.Errorf("%w: %w", context.DeadlineExceeded, ports.ErrChatHistoryLoadRejected)),
			code: "TARGET_HISTORY_LOAD_REJECTED", wantStarts: 1},
